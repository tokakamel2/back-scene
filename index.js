...
app.post("/supervisor/login", async (req, res) => {
  ...
  const token = jwt.sign({_id:user._id},config.get('jwtPrivateKey'),{ expiresIn: '24h' })
...
app.post("/rep/login", async (req, res) => {
  ...
  const validPassword = await bcrypt.compare(req.body.password, user.password);
  if (!validPassword) return res.status(400).send("Invalid email or password");
  const token = jwt.sign({_id:user._id,rep_id:user.rep_id},config.get('jwtPrivateKey'),{ expiresIn: '24h' })
